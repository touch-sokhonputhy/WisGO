...
// Cambodian destinations knowledge base for intelligent dynamic generation
...
// Flexible dynamic generator that builds structured plans based on query parameters
...
តើអ្នកចង់ឱ្យខ្ញុំរៀបចំគម្រោងដំណើរកម្សាន្តថ្មីសម្រាប់ខេត្តណាដែរ? (ឧទាហរណ៍៖ សៀមរាប ៣ ថ្ងៃ, កំពត-កែប ២ ថ្ងៃ, ភ្នំពេញ, បាត់ដំបង ឬ កោះរ៉ុង)?`
...
// Rapid 30-second backoff so temporary spikes recover promptly
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    try {
      ...
      console.warn('[Vercel WisGO AI] API client initialization notice:', clientErr);
    }
...
`;
    ...
    if (ai) {
      ...
      for (const model of candidateModels) {
        try {
          ...
          if (isBusy) {
            markModelBusy(model);
            console.log(`[Vercel WisGO AI] Model ${model} is currently busy; routing to next model.`);
          } else {
            console.log(`[Vercel WisGO AI] Model ${model} status: ${errMsg.slice(0, 80)}`);
          }
        }
      }
    }
    ...
    if (!replyText) {
      console.log('[Vercel WisGO AI] Serving intelligent dynamic local generator for query:', message);
