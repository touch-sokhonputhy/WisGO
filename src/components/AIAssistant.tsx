...
export const AIAssistant: React.FC<AIAssistantProps> = ({
  ...
  return (
    <div className="space-y-4">
      {/* Mobile Mode Segmented Switcher (Visible only on < lg) */}
      <div className="lg:hidden flex items-center p-1 bg-white border border-slate-200/90 rounded-2xl shadow-xs">
        <button
          type="button"
          onClick={() => setMobileTab('chat')}
          className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer min-h-[42px] touch-manipulation active:scale-98 ${
            mobileTab === 'chat'
        ...
        >
          ...
          <span>{language === 'km' ? 'ការសន្ទនា AI (Chat)' : 'AI Guide Chat'}</span>
        </button>
        <button
          type="button"
        ...
        >
          ...
          <span>{language === 'km' ? 'ឧបករណ៍ & រូបិយប័ណ្ណ' : 'Travel Tools & Rates'}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        {/* Main Chat Interface */}
        <div className={`lg:col-span-2 bg-white border border-slate-200 rounded-3xl shadow-xs flex-col h-[560px] sm:h-[650px] lg:h-[720px] overflow-hidden text-slate-800 ${
          mobileTab === 'chat' ? 'flex' : 'hidden lg:flex'
        }`}>
          
          {/* Header */}
          <div className="p-3 sm:p-5 bg-gradient-to-r from-[#F8FCFA] via-white to-[#F0FAF5] border-b border-slate-200 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <h3 className="font-bold text-[#1E293B] text-xs sm:text-base truncate">
                ...
                <p className="text-[10.5px] sm:text-xs text-slate-500 truncate">
                  {t('assistant.subtitle', 'Powered by Gemini AI')} • {language === 'km' ? 'ភាសា៖ ខ្មែរ' : 'English'} ({userProfile?.preferences?.preferredCurrency || 'USD'})
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              <button
                ...
                className="px-2.5 sm:px-3.5 py-2 min-h-[38px] rounded-xl bg-[#0B7A5C] hover:bg-[#086048] active:scale-95 text-white text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-xs transition-all shrink-0"
              ...
              {onOpenMyTrips && (
                <button
                  ...
                  className="px-2.5 sm:px-3 py-2 min-h-[38px] rounded-xl bg-slate-100 hover:bg-slate-200/80 active:scale-95 text-slate-700 text-xs font-bold flex items-center gap-1 cursor-pointer transition-all shrink-0"
              ...
              <button
                onClick={() => setMobileTab(mobileTab === 'chat' ? 'tools' : 'chat')}
                className={`lg:hidden p-2 min-h-[38px] min-w-[38px] rounded-xl text-xs font-bold flex items-center justify-center cursor-pointer transition-colors active:scale-95 ${
                  mobileTab === 'tools' ? 'bg-[#0B7A5C] text-white' : 'bg-[#DFF7ED] text-[#0B7A5C]'
                }`}
                title="Toggle Currency & Tools"
              >
                <Coins className="w-4 h-4" />
              </button>
            </div>
          ...
          <div className="p-2 sm:p-3 bg-slate-50 border-b border-slate-200 flex items-center gap-2 overflow-x-auto no-scrollbar touch-scroll">
            {quickPrompts.map((item, idx) => {
              ...
              return (
                <button
                  ...
                  disabled={loading}
                  className="px-3 py-1.5 min-h-[36px] rounded-xl bg-white hover:bg-[#DFF7ED] text-slate-700 hover:text-[#0B7A5C] border border-slate-200 text-xs font-semibold flex items-center gap-1.5 shrink-0 transition-all disabled:opacity-50 cursor-pointer shadow-2xs active:scale-95 whitespace-nowrap"
                >
          ...
          <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-4 bg-[#F8FCFA] touch-scroll">
            {messages.map(msg => {
              ...
              return (
                ...
                >
                  ...
                  {/* Bubble Container */}
                  <div className={`max-w-[92%] sm:max-w-[88%] ${msg.sender === 'user' ? 'text-right' : 'text-left'}`}>
                    <div className={`p-3.5 sm:p-4 rounded-2xl text-xs sm:text-sm leading-relaxed inline-block text-left ${
                      msg.sender === 'user'
          ...
          <div className="p-3 sm:p-4 bg-white border-t border-slate-200">
            ...
            >
              <input
                ...
                disabled={loading}
                className="flex-1 min-h-[46px] bg-[#F8FCFA] border border-slate-200 rounded-2xl px-3.5 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#0B7A5C]"
              />
              <button
                type="submit"
                disabled={!input.trim() || loading}
                className="p-3 min-h-[46px] min-w-[46px] flex items-center justify-center rounded-2xl bg-[#0B7A5C] hover:bg-[#086048] active:scale-95 text-white font-bold transition-all shadow-xs disabled:opacity-50 shrink-0 cursor-pointer"
              >
        ...
        <div className={`space-y-4 ${mobileTab === 'tools' ? 'block' : 'hidden lg:block'}`}>
          {/* Mobile Back to Chat banner */}
          <div className="lg:hidden flex items-center justify-between bg-white p-3.5 rounded-2xl border border-slate-200 shadow-2xs">
            <div>
              <h4 className="text-xs font-bold text-slate-800">Cambodia Travel Tools</h4>
              <p className="text-[11px] text-slate-500">Live currency rates & quick planner</p>
            </div>
            <button
              type="button"
              onClick={() => setMobileTab('chat')}
              className="px-3.5 py-1.5 rounded-xl bg-[#0B7A5C] text-white text-xs font-bold active:scale-95 cursor-pointer shadow-2xs"
            >
              Back to Chat
            </button>
          </div>
          ...
          <div className="p-4 sm:p-5 bg-gradient-to-br from-[#DFF7ED] to-white rounded-3xl border border-[#0B7A5C]/20 shadow-xs">
            ...
            <button
              type="button"
              onClick={() => setIsPlannerModalOpen(true)}
              className="mt-3.5 w-full py-2.5 min-h-[42px] rounded-xl bg-[#0B7A5C] hover:bg-[#086048] active:scale-95 text-white text-xs font-bold shadow-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
            >
